...
Tool-specific filenames (e.g. `CLAUDE.md`) are symlinks to this file. Edit `AGENTS.md`, never the
symlink itself. If a new tool needs its own conventional filename, add a symlink to it here rather
...
- When a change alters behaviour, setup, or a dependency that a docs page already describes,
  update that page in the same change. Don't let `./docs/site/src/content/docs/` go stale.
...
canonical in `./.claude/` and `./.agents/`.
