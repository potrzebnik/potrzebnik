...
function Pill({ label, hidden }: { label: string; hidden?: boolean }) {
  return (
    <span
      aria-hidden={hidden}
      className="border-benefits-strip-border bg-benefits-strip-pill-bg text-benefits-strip-fg shrink-0 rounded-full border px-6 py-3 text-sm font-medium whitespace-nowrap sm:text-base"
    >
      {label}
    </span>
  );
...
export default function BenefitsStripSection() {
  return (
    ...
    >
      <div className="animate-benefits-marquee flex w-max gap-4 motion-reduce:animate-none">
        {TAGS.map((label) => (
